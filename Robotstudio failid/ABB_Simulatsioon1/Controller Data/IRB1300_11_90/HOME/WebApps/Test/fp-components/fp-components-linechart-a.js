                    ...
                    for (let item of model) {
                        if (Array.isArray(item.points)) {
                            for (let p of item.points) {
                                if (autoXMax && (xMax === null || p[0] > xMax)) xMax = p[0];
                                if (autoXMin && (xMin === null || p[0] < xMin)) xMin = p[0];
                                if (autoYMax && (yMax === null || p[1] > yMax)) yMax = p[1];
                                if (autoYMin && (yMin === null || p[1] < yMin)) yMin = p[1];
                            }
                        }
                    }
                }
